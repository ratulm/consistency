...
The loop-finder subroutine can be implemented in various ways, for instance by using the Kosaraju-Sharir algorithm or Tarjan's strongly connected components algorithm \cite{reference_1_in_http://en.wikipedia.org/wiki/Tarjan's_strongly_connected_components_algorithm}.\ratul{describe the algorithm?}
...
Which of the two methods is preferable depends on the context and other procedures may exist as well. But we note that dependency graphs and forests are a general abstraction of network updates, and we return to it in Section~\ref{sec:discussion}.
